...
def start_training(model, X_train, y_train, X_validation, y_validation, scratch_training):
    # Step 1: Compile model
    if scratch_training == "Y":
        model.compile(
...
def model_preparation():
    train_features, train_labels, validation_features, validation_labels, test_features, test_labels = \
        data_preparation_cifar_resize(128, 128)
    # data_preparation_cifar_resize(128,128), data_preparation_cifar_original()
    model = build_model()
